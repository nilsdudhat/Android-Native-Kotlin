...
import com.demo.gallery.app.services.MediaService
...
class JobMainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (checkPermission()) {
            ...
            val jobInfo =
                JobInfo.Builder(Random.nextInt(), ComponentName(this@JobMainActivity, MediaService::class.java))
            job = jobInfo.setPersisted(true).build()
    ...
    private fun setUpRecyclerView(list : ArrayList<Media>) {
        binding.rvMedia.apply {
            if (layoutManager == null) {
                layoutManager = GridLayoutManager(this@JobMainActivity, 3)
            }
            if (this@JobMainActivity.adapter == null) {
                this@JobMainActivity.adapter = MediaAdapter()
                adapter = this@JobMainActivity.adapter
            }
            this@JobMainActivity.adapter!!.setMediaList(list)
        }
    }

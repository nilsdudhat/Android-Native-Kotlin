package com.belive.dating.activities.paywalls.topups.boost
...
import com.belive.dating.databinding.ActivityBoostPaywallBinding
...
class BoostPaywallActivity : NetworkReceiverActivity(), BoostPaywallAdapter.OnOfferClickListener {

    val binding by lazy {
        ActivityBoostPaywallBinding.inflate(layoutInflater)
    }

    lateinit var viewModel: BoostPaywallViewModel

    val adapter by lazy {
        BoostPaywallAdapter(viewModel, this)
    }

    override fun onResume() {
        super.onResume()

        mixPanel?.timeEvent(BoostPaywallActivity::class.java.simpleName)
    }

    override fun onPause() {
        super.onPause()

        mixPanel?.track(BoostPaywallActivity::class.java.simpleName)
    }
    ...
    private fun makePurchase() {
        try {
            LoadingDialog.show(this@BoostPaywallActivity)
    ...
    private fun getBoostOffers() {
        PaymentUtils.getOffers(productType = ProductType.BOOST, isLoading = {
            if (it) {
            ...
            Toast.makeText(this@BoostPaywallActivity, error.message, Toast.LENGTH_SHORT).show()
        }, onSuccess = { offers ->

            if (offers.isEmpty()) {
                Toast.makeText(this@BoostPaywallActivity, "Something went wrong...!", Toast.LENGTH_SHORT).show()
                onBackPressedDispatcher.onBackPressed()
                return@getOffers
            }
            ...
            viewModel.boostPlanList.set(offerList)
            ...
            viewModel.selectedOffer.set(viewModel.boostPlanList.get()!![defaultPosition])

            adapter.setSelectedPosition(defaultPosition)

            updateSelectedPlan()

            logger("--boosts--", gsonString(viewModel.boostPlanList.get()?.map { it.metadata["title"] }?.toList()))
    ...
    private fun setUpRecyclerView() {
        if (binding.rvBoostPlans.layoutManager == null) {
            binding.rvBoostPlans.layoutManager = LinearLayoutManager(this@BoostPaywallActivity, LinearLayoutManager.VERTICAL, false)
        }
        if (binding.rvBoostPlans.adapter == null) {
            binding.rvBoostPlans.adapter = this@BoostPaywallActivity.adapter
        }
    }
    ...
    private fun initViews() {
        ...
        viewModel.remainingBoostsCount.set(getUserPrefs().remainingBoosts.toString())
    }
    ...
    private fun CoroutineScope.observePurchase(paymentResource: Resource<PaymentResponse?>) {
        launch(Dispatchers.Main) {
            when (paymentResource.status) {
                ...
                Status.SIGN_OUT -> {
                    Toast.makeText(this@BoostPaywallActivity, "Your session has expired, Please log in again.", Toast.LENGTH_SHORT).show()

                    authOut()
                }

                Status.ADMIN_BLOCKED -> {
                    Toast.makeText(this@BoostPaywallActivity, "Admin has blocked you, because of security reasons.", Toast.LENGTH_SHORT).show()
                ...
                Status.ERROR -> {
                    ...
                    Toast.makeText(this@BoostPaywallActivity, paymentResource.message ?: "Something went wrong...!", Toast.LENGTH_SHORT).show()
                }
                ...
                Status.SUCCESS -> {
                    ...
                    if (data != null) {
                        ...
                        val purchaseItem = viewModel.selectedOffer.get()?.metadata?.get("boost_count")
                        Toast.makeText(this@BoostPaywallActivity, "$purchaseItem Super Likes has been Purchased", Toast.LENGTH_SHORT).show()
    ...
    private fun authOut() {
        ...
        authenticationHelper.signOut(
            ...
            onSuccess = {
                ...
                startActivity(Intent(this@BoostPaywallActivity, SignInActivity::class.java))
                finishAffinity()
                swipeLeft()
            },
        )
    ...
    override fun onInternetAvailableForFirstTime() {
        ...
        if (viewModel.boostPlanList.get() == null) {
            getBoostOffers()
        }
    }
    ...
    override fun onInternetConfigurationChanged(isConnected: Boolean) {
        ...
        if (isConnected && viewModel.boostPlanList.get() == null) {
            getBoostOffers()
        }
    }

    override fun onOfferClick(position: Int) {
        viewModel.selectedOffer.set(viewModel.boostPlanList.get()!![position])
        updateSelectedPlan()
    }

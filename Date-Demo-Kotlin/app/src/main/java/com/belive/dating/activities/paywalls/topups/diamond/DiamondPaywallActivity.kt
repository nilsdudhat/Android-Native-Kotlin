...
import com.belive.dating.databinding.ActivityDiamondPaywallBinding
...
class DiamondPaywallActivity : NetworkReceiverActivity(), DiamondPaywallAdapter.OnOfferClickListener {

    val binding by lazy {
        ActivityDiamondPaywallBinding.inflate(layoutInflater)
    }

    lateinit var viewModel: DiamondPaywallViewModel

    val adapter by lazy {
        DiamondPaywallAdapter(viewModel, this)
    }

    override fun onResume() {
        super.onResume()

        mixPanel?.timeEvent(DiamondPaywallActivity::class.java.simpleName)
    }

    override fun onPause() {
        super.onPause()

        mixPanel?.track(DiamondPaywallActivity::class.java.simpleName)
    }
    ...
    private fun makePurchase() {
        try {
            LoadingDialog.show(this@DiamondPaywallActivity)
    ...
    private fun getDiamondOffers() {
        PaymentUtils.getOffers(productType = ProductType.DIAMOND, isLoading = {
            if (it) {
            ...
            Toast.makeText(this@DiamondPaywallActivity, error.message, Toast.LENGTH_SHORT).show()
        }, onSuccess = { offers ->

            if (offers.isEmpty()) {
                Toast.makeText(this@DiamondPaywallActivity, "Something went wrong...!", Toast.LENGTH_SHORT).show()
                onBackPressedDispatcher.onBackPressed()
                return@getOffers
            }
            ...
            viewModel.diamondPlanList.set(offerList)
            ...
            viewModel.selectedOffer.set(viewModel.diamondPlanList.get()!![defaultPosition])

            adapter.setSelectedPosition(defaultPosition)

            updateSelectedPlan()

            logger("--diamonds--", gsonString(viewModel.diamondPlanList.get()?.map { it.metadata["title"] }?.toList()))
    ...
    private fun setUpRecyclerView() {
        if (binding.rvDiamondPlans.layoutManager == null) {
            binding.rvDiamondPlans.layoutManager = LinearLayoutManager(this@DiamondPaywallActivity, LinearLayoutManager.VERTICAL, false)
        }
        if (binding.rvDiamondPlans.adapter == null) {
            binding.rvDiamondPlans.adapter = this@DiamondPaywallActivity.adapter
        }
    }
    ...
    private fun initViews() {
        ...
        viewModel.remainingDiamondsCount.set(getUserPrefs().remainingDiamonds.toString())
    }
    ...
    private fun CoroutineScope.observePurchase(paymentResource: Resource<PaymentResponse?>) {
        launch(Dispatchers.Main) {
            when (paymentResource.status) {
                ...
                Status.SIGN_OUT -> {
                    Toast.makeText(this@DiamondPaywallActivity, "Your session has expired, Please log in again.", Toast.LENGTH_SHORT).show()

                    authOut()
                }

                Status.ADMIN_BLOCKED -> {
                    Toast.makeText(this@DiamondPaywallActivity, "Admin has blocked you, because of security reasons.", Toast.LENGTH_SHORT).show()
                ...
                Status.ERROR -> {
                    ...
                    Toast.makeText(this@DiamondPaywallActivity, paymentResource.message ?: "Something went wrong...!", Toast.LENGTH_SHORT).show()
                }
                ...
                Status.SUCCESS -> {
                    ...
                    if (data != null) {
                        ...
                        val purchaseItem = viewModel.selectedOffer.get()?.metadata?.get("diamond_count")
                        Toast.makeText(this@DiamondPaywallActivity, "$purchaseItem Diamonds has been Purchased", Toast.LENGTH_SHORT).show()
    ...
    private fun authOut() {
        ...
        authenticationHelper.signOut(
            ...
            onSuccess = {
                ...
                startActivity(Intent(this@DiamondPaywallActivity, SignInActivity::class.java))
                finishAffinity()
                swipeLeft()
            },
        )
    ...
    override fun onInternetAvailableForFirstTime() {
        ...
        if (viewModel.diamondPlanList.get() == null) {
            getDiamondOffers()
        }
    }
    ...
    override fun onInternetConfigurationChanged(isConnected: Boolean) {
        ...
        if (isConnected && viewModel.diamondPlanList.get() == null) {
            getDiamondOffers()
        }
    }

    override fun onOfferClick(position: Int) {
        viewModel.selectedOffer.set(viewModel.diamondPlanList.get()!![position])
        updateSelectedPlan()
    }

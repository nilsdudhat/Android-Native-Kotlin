...
open class DoubleValueSeekBarView : View {
    ...
    companion object {
        ...
        private const val MIN_AGE_RANGE = 5
    }
    ...
    private var minStep: Int = DEFAULT_MIN_STEP_VALUE
        set(value) {
            field = value
            invalidate()
            requestLayout()
        }
    private var maxStep: Int = DEFAULT_MAX_STEP_VALUE
        set(value) {
            field = value
            invalidate()
            requestLayout()
    ...
    private fun parseAttr(attrs: AttributeSet?) {
        val typedArray =
            context.theme.obtainStyledAttributes(attrs, R.styleable.DoubleValueSeekBarView, 0, 0)
        ...
        if (typedArray.hasValue(R.styleable.DoubleValueSeekBarView_r2barHeight)) {
            barHeight =
                typedArray.getDimensionPixelSize(R.styleable.DoubleValueSeekBarView_r2barHeight, 0)
        }

        if (typedArray.hasValue(R.styleable.DoubleValueSeekBarView_r2circleRadius)) {
            circleRadius = typedArray.getDimensionPixelSize(
                R.styleable.DoubleValueSeekBarView_r2circleRadius,
                0
            )
        ...
        if (typedArray.hasValue(R.styleable.DoubleValueSeekBarView_r2circleTextSize)) {
            circleTextSize = typedArray.getDimensionPixelSize(
                R.styleable.DoubleValueSeekBarView_r2circleTextSize,
                0
            )
        }
        if (typedArray.hasValue(R.styleable.DoubleValueSeekBarView_r2circleTextColor)) {
            circleTextColor = typedArray.getColor(
                R.styleable.DoubleValueSeekBarView_r2circleTextColor,
                DEFAULT_TEXT_COLOR
            )
        }
        ...
        if (typedArray.hasValue(R.styleable.DoubleValueSeekBarView_r2CurrentMaxValue)) {
            currentMaxValue =
                typedArray.getInt(R.styleable.DoubleValueSeekBarView_r2CurrentMaxValue, 100)
        }

        if (typedArray.hasValue(R.styleable.DoubleValueSeekBarView_r2CurrentMinValue)) {
            currentMinValue =
                typedArray.getInt(R.styleable.DoubleValueSeekBarView_r2CurrentMinValue, 0)
        }
    ...
    private fun drawDoubleValueBarHorizontal(canvas: Canvas) {
        ...
        val percentFilledMin =
            calculateProgress(minValueToDraw, minValue, maxValue).toFloat() / DEFAULT_MAX_VALUE
        val fillLengthMin = barLength * percentFilledMin
        val fillPositionMin = left + fillLengthMin


        val percentFilledMax =
            calculateProgress(maxValueToDraw, minValue, maxValue).toFloat() / DEFAULT_MAX_VALUE
        ...
        when (action and ACTION_MASK) {
            ...
            ACTION_MOVE -> {
                ...
                if (pressedThumb != null) {
                    ...
                    mOnDoubleValueSeekBarViewChangeListener?.onValueChanged(
                        this@DoubleValueSeekBarView,
                        currentMinValue,
                        currentMaxValue,
                        true
                    )
            ...
            ACTION_UP -> {
                ...
                if (isDragging) {
                    ...
                    mOnDoubleValueSeekBarViewChangeListener?.onStopTrackingTouch(
                        this@DoubleValueSeekBarView,
                        currentMinValue,
                        currentMaxValue
                    )
            ...
            ACTION_POINTER_DOWN -> {
                logger("--move--", "ACTION_POINTER_DOWN")

                //invalidate()
            }

            ACTION_POINTER_UP -> {
                logger("--move--", "ACTION_POINTER_UP")

                invalidate()
            }

            ACTION_CANCEL -> {
                logger("--move--", "ACTION_CANCEL")

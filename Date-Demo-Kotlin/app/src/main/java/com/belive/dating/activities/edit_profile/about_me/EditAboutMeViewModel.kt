...
class EditAboutMeViewModel(val savedStateHandle: SavedStateHandle) : ViewModel() {

    val TAG = "EDIT_ABOUT_ME_VIEW_MODEL"
    ...
    fun saveAboutMe(): MutableStateFlow<Resource<UserResponse?>> {
        val data = MutableStateFlow<Resource<UserResponse?>>(Resource.loading(null))

        job = viewModelScope.launch(Dispatchers.IO) {

            val json = JsonObject()
            json.addProperty("about_me", aboutMe.get())

            val response = userRepository.updateUserDetails(json)
            val errorBody = response.errorBody()?.string()

            logger("--about_me--", "request.url: ${response.raw().request.url}")
            logger("--about_me--", "request.body: ${gsonString(response.raw().request.body)}")
            logger("--about_me--", "code: ${response.code()}")
            logger("--about_me--", "isSuccessful: ${response.isSuccessful}")
            logger("--about_me--", "errorBody: $errorBody")
            logger("--about_me--", "body: ${gsonString(response.body())}")
            logger("--about_me--", "query: ${gsonString(json)}")
    ...
    fun validateName(): MutableStateFlow<Resource<JsonObject?>> {
        ...
        job = viewModelScope.launch(Dispatchers.IO) {
            ...
            val response = introductionService.validateText(aboutMe.get()?.trim())
            val errorBody = response.errorBody()?.string()

...
class BoostPaywallViewModel(val savedStateHandle: SavedStateHandle) : ViewModel() {

    val TAG = "BOOST_PAYWALL_VIEW_MODEL"

    val isLoading = ObservableField(true)
    val selectedOffer = ObservableField<Offering>()
    val remainingBoostsCount = ObservableField<String>()
    val buttonText = ObservableField<String>()
    val boostPlanList = ObservableField<ArrayList<Offering>>()

    fun updateState() {
        savedStateHandle["${TAG}_isLoading"] = isLoading.get()
        savedStateHandle["${TAG}_selectedOffer"] = gsonString(selectedOffer.get())
        savedStateHandle["${TAG}_remainingBoostsCount"] = remainingBoostsCount.get()
        savedStateHandle["${TAG}_buttonText"] = buttonText.get()
        savedStateHandle["${TAG}_boostPlanList"] = gsonString(boostPlanList.get())
    }
    ...
    fun getState() {
        ...
        remainingBoostsCount.set(savedStateHandle["${TAG}_remainingBoostsCount"])
        buttonText.set(savedStateHandle["${TAG}_buttonText"])
        boostPlanList.set(savedStateHandle.get<String>("${TAG}_boostPlanList")?.fromJson<ArrayList<Offering>>())
    }

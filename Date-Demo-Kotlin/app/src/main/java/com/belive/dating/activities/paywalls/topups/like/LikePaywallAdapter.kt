...
import com.belive.dating.databinding.ItemLikePaywallBinding
import com.belive.dating.extensions.logger
import com.revenuecat.purchases.Offering
import java.text.DecimalFormatSymbols
import java.util.Locale

class LikePaywallAdapter(val viewModel: LikePaywallViewModel, private val onOfferClickListener: OnOfferClickListener) :
    RecyclerView.Adapter<LikePaywallAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ItemLikePaywallBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return ViewHolder(binding)
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        logger("--likes--", "position: $position")
        ...
        val likeCount = (offering.metadata["like_count"] as String).toInt()
        val priceInDouble = convertFormattedPriceToDouble(offerPrice)
        val pricePerLike = String.format(Locale.getDefault(), "%.2f", priceInDouble / likeCount).replace(".00", "")
        holder.binding.pricePerLike = StringBuilder().append(offerPrice[0]).append(pricePerLike).append(" / Like").toString()
    ...
    override fun getItemCount(): Int {
        logger("--likes--", "getItemCount: ${paywallList.size}")
    ...
    class ViewHolder(val binding: ItemLikePaywallBinding) : RecyclerView.ViewHolder(binding.root)
}
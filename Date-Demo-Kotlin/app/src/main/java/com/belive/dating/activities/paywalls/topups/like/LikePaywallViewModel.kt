...
class LikePaywallViewModel(val savedStateHandle: SavedStateHandle) : ViewModel() {

    val TAG = "LIKE_PAYWALL_VIEW_MODEL"
    ...
    val remainingLikesCount = ObservableField<String>()
    val buttonText = ObservableField<String>()
    val likePlanList = ObservableField<ArrayList<Offering>>()
    ...
    fun updateState() {
        ...
        savedStateHandle["${TAG}_remainingLikesCount"] = remainingLikesCount.get()
        savedStateHandle["${TAG}_buttonText"] = buttonText.get()
        savedStateHandle["${TAG}_likePlanList"] = gsonString(likePlanList.get())
    }
    ...
    fun getState() {
        ...
        remainingLikesCount.set(savedStateHandle["${TAG}_remainingLikesCount"])
        buttonText.set(savedStateHandle["${TAG}_buttonText"])
        likePlanList.set(savedStateHandle.get<String>("${TAG}_likePlanList")?.fromJson<ArrayList<Offering>>())
    }
    ...
    fun getRewindLikeConversionDetails(): MutableStateFlow<Resource<RewindLikeConversionResponse?>> {
        ...
        job = viewModelScope.launch(Dispatchers.IO) {
            try {
                val response = userRepository.getRewindLikeConversionDetails("like")
                val errorBody = response.errorBody()?.string()

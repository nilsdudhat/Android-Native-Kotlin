...
import com.belive.dating.databinding.ItemBoostPaywallBinding
import com.belive.dating.extensions.logger
import com.revenuecat.purchases.Offering
import java.text.DecimalFormatSymbols
import java.util.Locale

class BoostPaywallAdapter(
    val viewModel: BoostPaywallViewModel,
    private val onOfferClickListener: OnOfferClickListener,
) :
    RecyclerView.Adapter<BoostPaywallAdapter.ViewHolder>() {
    ...
    var paywallList = arrayListOf<Offering>()
        set(value) {
            logger("--boosts--", "update")
            field = value
            notifyItemRangeChanged(0, value.size)
        }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ItemBoostPaywallBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return ViewHolder(binding)
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        logger("--boosts--", "position: $position")
        ...
        val boostCount = (offering.metadata["boost_count"] as String).toInt()
        ...
        holder.binding.pricePerBoost = StringBuilder().append(offerPrice[0]).append(pricePerLike).append(" / Boost").toString()
    ...
    override fun getItemCount(): Int {
        logger("--boost--", "getItemCount: ${paywallList.size}")
    ...
    class ViewHolder(val binding: ItemBoostPaywallBinding) : RecyclerView.ViewHolder(binding.root)
}